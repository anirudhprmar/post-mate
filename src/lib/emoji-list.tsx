...
const EmojiList = forwardRef<EmojiListHandle, EmojiListProps>((props, ref) => {
  ...
  return (
    <div className="border-border bg-popover z-50 min-w-[10rem] overflow-hidden rounded-md border p-1 shadow-md">
      {props.items.map((item, index) => (
        <Tooltip key={item.name}>
          <TooltipTrigger asChild>
            <button
              onClick={() => selectItem(index)}
              className={`flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-sm transition-colors outline-none ${
                index === selectedIndex
