...
export default function DraftsPage() {
    ...
    const tabs = ["All Drafts", "Instagram", "LinkedIn", "X", "Threads", "Facebook", "Youtube"]
    return (
            ...
            <Tabs defaultValue="all" className="w-full">
                <div className="px-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
                    <TabsList className="bg-background/40 backdrop-blur-xl border border-border/40 p-1.5 rounded-full inline-flex md:flex shadow-sm">
                        {
                            tabs.map((tab) => (
                                <TabsTrigger key={tab} value={tab === "All Drafts" ? "all" : tab.toLowerCase()} className="rounded-full px-6 data-[state=active]:bg-primary/10 data-[state=active]:text-primary data-[state=active]:shadow-sm transition-all duration-300">
                                    {tab}
                                </TabsTrigger>
                            ))
                        }
                                                ...
                                                <Link key={draft.id} href={`/dashboard/post/${draft.id}`} className="block">
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center justify-between mt-6 text-sm text-muted-foreground/80 font-medium">
                                                            ...
                                                            <Badge
                                                                variant={draft.status === 'draft' ? 'default' : (draft.status === "published" ? "default" : "secondary")}
